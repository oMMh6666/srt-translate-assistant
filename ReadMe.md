...
SRT 字幕「反思式翻译」工作台：Web UI + FastAPI，通过 Gemini 官方 `google-genai` 库调用（非 OpenAI 兼容接口）。
...
浏览器自动打开 <http://127.0.0.1:8778/>。端口在 `core/config.py` 的 `PORT` 里改。
...
| Key 管理 | 行内编辑、每行勾选框 + 顶部全选，启用 / 禁用 / 删除 / 重置用量只作用于勾选的行。**被禁用的 Key 在列表里带删除线**，一眼能区分 |
...
| 默认参数 | 唯一来源是 `core/config.py` 的 `DEFAULT_CONFIG`（模型 / 温度 / 批次大小 / 目标语言 / 超时等），不读外部配置文件；要改默认值直接改这个文件 |
...
**推荐使用 Gemini 3.7 / 3.8，效果明显更优**，但免费账号配额有限（一天约 20 次）。程序会自动轮换使用次数最少的 Key。
